...
function scrapeResortData(url, result) {
  return Nightmare()
    ...
    .evaluate(() => {
      ...
      const impDateCells = document.querySelectorAll('#resort_impdates li');  //eslint-disable-line
      ...
      const relRegionItems = document.querySelectorAll('.rel_regions a');  //eslint-disable-line

      const stateName = [...relRegionItems].find((item) => {
        let thisState;
        const states = [
          {
            state_name: 'Alabama',
